...
void MagicCalmFilter::onInitialized() {
    ...
    for (int i = 0; i < 256; ++i) {
        ...
        arrayOfByte[(1+i*4)]= static_cast<unsigned char>(arrayOfInt2[i]);
        arrayOfByte[(2+i*4)]= static_cast<unsigned char>(arrayOfInt3[i]);
        arrayOfByte[(3+i*4)]= static_cast<unsigned char>(arrayOfInt4[i]);
    }
    ...
    for (int j = 0; j < 256; ++j) {
        arrayOfByte[(1024 + j * 4)] = static_cast<unsigned char>(arrayOfInt5[j]);
        arrayOfByte[(1 + (1024 + j * 4))] = static_cast<unsigned char>(arrayOfInt5[j]);
        arrayOfByte[(2 + (1024 + j * 4))] = static_cast<unsigned char>(arrayOfInt5[j]);
