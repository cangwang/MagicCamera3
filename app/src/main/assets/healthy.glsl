...
{ 
	...
	color.g = texture(sampler, vec2(color.g, 0.)).r;
	color.b = texture(sampler, vec2(color.b, 0.)).r;
	return color;
} 

vec4 level1c(vec4 color, sampler2D sampler) 
{ 
	color.r = texture(sampler, vec2(color.r, 0.)).g;
	color.g = texture(sampler, vec2(color.g, 0.)).g;
	color.b = texture(sampler, vec2(color.b, 0.)).g;
	return color;
} 

vec4 level2c(vec4 color, sampler2D sampler) 
{ 
	color.r = texture(sampler, vec2(color.r,0.)).b; 
	color.g = texture(sampler, vec2(color.g,0.)).b;
...
{ 
	...
	vec4 t0 = texture(mask, vec2(textureCoordinate.x, textureCoordinate.y)); 
	
	// naver skin 
	vec4 c2 = texture(inputImageTexture, textureCoordinate); 
	vec4 c5 = c2; 
