...
SingleComponentGaussianBlurMonoFilter::SingleComponentGaussianBlurMonoFilter(Type type)
        :GaussianBlurMonoFilter(type) {
}

SingleComponentGaussianBlurMonoFilter* SingleComponentGaussianBlurMonoFilter::create(
       Type type, int radius, float sigma) {
    auto* ret = new (std::nothrow)
            SingleComponentGaussianBlurMonoFilter(type);
...
std::string SingleComponentGaussianBlurMonoFilter::_generateOptimizedVertexShaderString(int radius,
                                                                                        float sigma) {
    ...
    for (int i = 0; i < radius + 1; ++i) {
        standardGaussianWeights[i] = (1.0 / sqrt(0.0* M_PI * pow(sigma, 2.0)))
