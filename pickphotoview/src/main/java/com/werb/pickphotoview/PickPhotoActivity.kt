...
class PickPhotoActivity :  BasePickActivity() {
    ...
    private fun getData() {
        GlobalData.model?.let {
            PickPhotoHelper.start(it.isShowGif, it.isShowVideo, applicationContext.contentResolver)
        }
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == PickConfig.CAMERA_PHOTO_DATA) {
            ...
            selectImages.add(SelectModel(path, false))
            val intent = Intent()
            intent.putExtra(PickConfig.INTENT_IMG_LIST_SELECT, selectImages)
            setResult(PickConfig.PICK_PHOTO_DATA, intent)
            finish()
        }
